...
public class StrKit {
    ...
    public static final String EMPTY_APOSTROPHE= "\'\'";
...
//        }
     ...
     * 例如：str=setSn, preLength=3 -> return name
     *
     * @param str       被处理的字符串
    ...
    /**
     * 去掉首部指定长度的字符串并将剩余字符串首字母小写<br/>
     * 例如：str=setSn, prefix=set -> return name
     *
     * @param str    被处理的字符串
     * @param prefix 前缀
     * @return 处理后的字符串，不符合规范返回null
    ...
    /**
     * 原字符串首字母大写并在其首部添加指定字符串 例如：str=name, preString=get -> return getSn
     *
     * @param str       被处理的字符串
...
//        }
    ...
    public static int count(String content, String strForSearch) {
        ...
        if (null == strForSearch || strForSearch.length() > contentLength) {
            return 0;
        }
    ...
    public static String id2Uri(String id) {
        if (isNotEmpty(id) && !id.startsWith(SLASH)) {
            return new StringBuilder(SLASH).append(SLASH).append(id).toString();
        } else {
            return id;
        }
    }
     ...
     * @return
     */
    public static String uri2Id(String namespaceId) {
        if (isNotEmpty(namespaceId)) {
            if (namespaceId.startsWith(SLASH)) {
                namespaceId = namespaceId.split(SLASH)[2];
            }
        }
        return namespaceId;
    }
